...
| ---: | --- | --- | --- | --- | --- | --- | ---: | --- |
| 1 | 0:00–0:45 | Cover／開場提問 | 一段程式碼該放哪裡，是長期協作與理解成本問題 | 單一程式碼片段被多個目錄拉扯 | 講者詮釋 | Static | 45 | 從標題進入三階段演講路線圖 |
| 2 | 0:45–1:30 | Talk roadmap | Technical-based 解決技術分類；Feature-based 改善業務聚合；FSD 再補上共同規則與可機械驗證的保護 | Technical-based → Feature-based → FSD 三欄演進；FSD 欄旁以文字、圖示與框線標示 Skill 與 Steiger／CI 的有限責任 | 講者詮釋／現行 guidance | Static | 45 | 從三階段路線圖進入 Technical-based 詳細介紹 |
| 3 | 1:30–3:30 | Technical-based baseline | 技術分類容易上手，但業務修改邊界會散落 | `components`／`composables`／`utils` 跨目錄追查 | 講者詮釋 | Static | 120 | 承接路線圖第一站，公平承認起點，再尋找 locality |
...
第 2 頁的三欄是演講路線圖，不是所有專案的唯一目錄形狀、替代順序或成熟度排名。Feature-based 不等於固定資料夾規格；Skill 是人與 AI 的共同判斷語言，不是 autonomous architecture judge；Steiger／CI 只保護可被自動化觀察的規則，不證明完整 architecture 或 business semantics。不得宣稱 FSD 是唯一解。
...
- 不修改 package manifest、OpenSlide config、themes、其他 slides、Wayfinder map、其他 issues／research 或 domain context；本次第 2 頁路線圖修訂只允許同步本 spec 與 Ticket 11。
...
- 執行 scoped diff audit，確認只修改正式 deck 與本次明確允許同步的 spec／Ticket 11，沒有更動 package manifest、OpenSlide config、themes、其他 slides、其他 planning／research 或 domain files。
...
- 除本次明確允許同步的 spec 與 Ticket 11 外，不修改 Wayfinder map／其他 issues／research、CONTEXT、themes、其他 slides、package manifest 或 OpenSlide config。
